...
if [[ "19.03" > $docker_version ]]; then
  docker run --entrypoint ./docker/run.sh -ti -it -p 6016-6025:6006-6015 --name mseed --rm seed_rl:$ENVIRONMENT $ENVIRONMENT $AGENT $NUM_ACTORS $ENV_BATCH_SIZE $@
else
  docker run --gpus all --entrypoint ./docker/run.sh -ti -it -p 6016-6025:6006-6015 -e HOST_PERMS="$(id -u):$(id -g)" --name mseed --rm seed_rl:$ENVIRONMENT $ENVIRONMENT $AGENT $NUM_ACTORS $ENV_BATCH_SIZE $@
fi
