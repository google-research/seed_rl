...
# Training.
flags.DEFINE_integer('total_environment_episodes', int(1e5),
...
flags.DEFINE_string('init_model', None,
                    'Path to the model used to initialize the agent.')
# Loss settings.
...
def learner_loop(create_env_fn, create_agent_fn, create_optimizer_fn):
  ...
  with strategy.scope():
    ...
    final_iteration = FLAGS.total_environment_episodes
    ...
    with tf.device(host):
      ...
      def create_inference_fn(inference_device):
        ...
        def inference(env_ids, run_ids, env_outputs, raw_rewards):
          ...
          done_eps += len(done_ids)
          print("done eps: ", done_eps)
          if i == 0:
  ...
  #   minimize(it)
  while done_eps < final_iteration:
    # minimize(it)
    pass
  # logger.shutdown()
  # manager.save()
