...
RUN_NAME="${ENVIRONMENT}_${AGENT}_${NUM_ACTORS}_${ENV_PER_ACTOR}_${SUB_TASK}_${PORT}_${CUDA}_${RUN_ID}"
LOG_DIR="/outdata/logs/seed_rl/${ENVIRONMENT}_${AGENT}/${SUB_TASK}/${NUM_ACTORS}_${ENV_PER_ACTOR}_${PORT}_${CUDA}_${RUN_ID}"
# shift 4
shift 3
if [[ $1 ]]; then
...
if [[ "19.03" > $docker_version ]]; then
  ...
  docker run --gpus '"device='"${CUDA}"'"' -v ~/:/outdata --entrypoint ./docker/run_${ENVIRONMENT}.sh -ti -it -p ${PORT}-${PORT}:${PORT}-${PORT} -e HOST_PERMS="$(id -u):$(id -g)" --name ${RUN_NAME} --rm seed_rl:$ENVIRONMENT $ENVIRONMENT $AGENT $NUM_ACTORS $ENV_BATCH_SIZE $LOG_DIR $PORT $SUB_TASK $@
fi
